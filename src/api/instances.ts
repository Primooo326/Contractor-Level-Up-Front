...
export const fetchApiBase = {
    ...
    downloadFile: (url: string, body: any, _instance: "base" = "base", maxRetries?: number) =>
        handleRequest(
            () => instance(_instance)
                .post(url, body, { responseType: 'blob' })
                .then(responseBody),
    ...
    downloadFileGet: (url: string, _instance: "base" = "base", maxRetries?: number) =>
        handleRequest(
            () => instance(_instance)
                .get(url, { responseType: 'blob' })
                .then(responseBody),
